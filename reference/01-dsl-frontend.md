...
4. **Output/Source ref**: char is `o` or `s` AND `src[i+1]` is a digit. Greedily consume following digits. `o`→`OUTPUT_REF`, `s`→`SOURCE_REF`. Lexeme e.g. `o0`, `s3`. **NOTE**: for `OUTPUT_REF`, if not a member segment (`tokens[-1].type === 'DOT'`), lexeme must match `/^o[0-7]$/` or throws `SyntaxError` ("Output surface reference '<lexeme>' is out of range; expected o0-o7 at line L col C").
