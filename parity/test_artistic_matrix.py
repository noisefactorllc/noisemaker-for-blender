...
class ArtisticMatrixTests(unittest.TestCase):
    ...
    def test_extrude_type_by_depth_source(self):
        self.assert_matrix("extrude", ("type", "depthSource"), {(t, d) for t in ("blocks", "pyramids") for d in ("luminance", "random")})

    def test_halftone_live_mode_by_pattern(self):
        self.assert_matrix("halftone", ("mode", "pattern"), {("color", "dot"), ("mono", "dot"), ("mono", "line"), ("mono", "circle")})

    def test_morphology_mode_by_shape(self):
        self.assert_matrix("morphology", ("mode", "shape"), {(m, s) for m in ("dilate", "erode") for s in ("square", "round")})

    def test_wind_method_by_direction(self):
        self.assert_matrix("wind", ("method", "direction"), {(m, d) for m in ("wind", "blast", "stagger") for d in ("fromLeft", "fromRight")})

    def test_error_diffusion_branch(self):
        self.assertIn("errorDiffusion", {row[0] for row in fixture_values("dither", ("type",))})
