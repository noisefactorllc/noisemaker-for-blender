...
float nm_constant(vec2 st, float freq, float speed) {
...
// Quality: Quadratic B-spline (degree 2) smoothing, minimum 3×3 nm_kernel to avoid lattice artifacts
...
float quadratic3x3Value(vec2 st, float freq, float speed) {
    ...
    // Row -1 (y-1)
    float v00 = nm_constant(st + vec2(-nd, -nd), freq, speed);
    float v10 = nm_constant(st + vec2(0.0, -nd), freq, speed);
    float v20 = nm_constant(st + vec2(nd, -nd), freq, speed);
    
    // Row 0 (y)
    float v01 = nm_constant(st + vec2(-nd, 0.0), freq, speed);
    float v11 = nm_constant(st, freq, speed);
    float v21 = nm_constant(st + vec2(nd, 0.0), freq, speed);
    
    // Row 1 (y+1)
    float v02 = nm_constant(st + vec2(-nd, nd), freq, speed);
    float v12 = nm_constant(st + vec2(0.0, nd), freq, speed);
    float v22 = nm_constant(st + vec2(nd, nd), freq, speed);
...
float bicubicValue(vec2 st, float freq, float speed) {
    ...
    float x0y0 = nm_constant(vec2(u0, v0), freq, speed);
    float x0y1 = nm_constant(vec2(u0, v1), freq, speed);
    float x0y2 = nm_constant(vec2(u0, v2), freq, speed);
    float x0y3 = nm_constant(vec2(u0, v3), freq, speed);

    float x1y0 = nm_constant(vec2(u1, v0), freq, speed);
    float x1y1 = nm_constant(st, freq, speed);
    float x1y2 = nm_constant(vec2(u1, v2), freq, speed);
    float x1y3 = nm_constant(vec2(u1, v3), freq, speed);

    float x2y0 = nm_constant(vec2(u2, v0), freq, speed);
    float x2y1 = nm_constant(vec2(u2, v1), freq, speed);
    float x2y2 = nm_constant(vec2(u2, v2), freq, speed);
    float x2y3 = nm_constant(vec2(u2, v3), freq, speed);

    float x3y0 = nm_constant(vec2(u3, v0), freq, speed);
    float x3y1 = nm_constant(vec2(u3, v1), freq, speed);
    float x3y2 = nm_constant(vec2(u3, v2), freq, speed);
    float x3y3 = nm_constant(vec2(u3, v3), freq, speed);
...
float value(vec2 st, float freq, int interp, float speed) {
    ...
    float x1y2 = nm_constant(vec2(st.x, st.y + ndY), freq, speed);
    float x2y1 = nm_constant(vec2(st.x + ndX, st.y), freq, speed);
    float x2y2 = nm_constant(vec2(st.x + ndX, st.y + ndY), freq, speed);
