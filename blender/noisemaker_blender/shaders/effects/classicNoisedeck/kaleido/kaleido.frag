...
float nm_constant(vec2 st, float freq) {
...
// Replaces legacy bicubic 4×4 (16 taps) with 3×3 nm_kernel (9 taps)
...
float quadratic3x3Value(vec2 st, float freq) {
    ...
    // Row -1 (y-1)
    float v00 = nm_constant(st + vec2(-nd, -nd), freq);
    float v10 = nm_constant(st + vec2(0.0, -nd), freq);
    float v20 = nm_constant(st + vec2(nd, -nd), freq);
    
    // Row 0 (y)
    float v01 = nm_constant(st + vec2(-nd, 0.0), freq);
    float v11 = nm_constant(st, freq);
    float v21 = nm_constant(st + vec2(nd, 0.0), freq);
    
    // Row 1 (y+1)
    float v02 = nm_constant(st + vec2(-nd, nd), freq);
    float v12 = nm_constant(st + vec2(0.0, nd), freq);
    float v22 = nm_constant(st + vec2(nd, nd), freq);
...
float bicubicValue(vec2 st, float freq) {
    ...
    float x0y0 = nm_constant(vec2(u0, v0), freq);
    float x0y1 = nm_constant(vec2(u0, v1), freq);
    float x0y2 = nm_constant(vec2(u0, v2), freq);
    float x0y3 = nm_constant(vec2(u0, v3), freq);

    float x1y0 = nm_constant(vec2(u1, v0), freq);
    float x1y1 = nm_constant(st, freq);
    float x1y2 = nm_constant(vec2(u1, v2), freq);
    float x1y3 = nm_constant(vec2(u1, v3), freq);

    float x2y0 = nm_constant(vec2(u2, v0), freq);
    float x2y1 = nm_constant(vec2(u2, v1), freq);
    float x2y2 = nm_constant(vec2(u2, v2), freq);
    float x2y3 = nm_constant(vec2(u2, v3), freq);

    float x3y0 = nm_constant(vec2(u3, v0), freq);
    float x3y1 = nm_constant(vec2(u3, v1), freq);
    float x3y2 = nm_constant(vec2(u3, v2), freq);
    float x3y3 = nm_constant(vec2(u3, v3), freq);
...
float value(vec2 st, float freq, int interp) {
    ...
    float x1y2 = nm_constant(vec2(st.x, st.y + ndY), freq);
    float x2y1 = nm_constant(vec2(st.x + ndX, st.y), freq);
    float x2y2 = nm_constant(vec2(st.x + ndX, st.y + ndY), freq);
...
vec3 convolve(vec2 uv, float nm_kernel[9], bool divide) {
    ...
    for(int i = 0; i < 9; i++){
        ...
        // keep a running tally of the nm_kernel weights
        kernelWeight += nm_kernel[i];
    }

    // normalize the convolution by dividing by the nm_kernel weight
    if (divide) {
...
// Per-KERNEL convolution branch — only the active nm_kernel for the current
