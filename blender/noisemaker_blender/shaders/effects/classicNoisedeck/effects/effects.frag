...
// Convolution nm_kernel branch — only the active nm_kernel for the current EFFECT
// gets compiled in. Called from main() inside the EFFECT == nm_kernel #elif
// branch, so this function only needs to emit one nm_kernel body.
...
// CGA - MIT License
// https://github.com/spite/Wagner/blob/master/nm_fragment-shaders/cga-fs.glsl
...
// Bloom - MIT License
// Modified from https://github.com/spite/Wagner/blob/master/nm_fragment-shaders/bloom-fs.glsl
vec3 bloom(vec2 st) {
    vec3 sum = vec3(0.0);
    vec3 color = vec3(0.0);
...
#else
        // convolution nm_kernel branches handled inside convolutionEffect()
