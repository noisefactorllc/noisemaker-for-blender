...
// Preserve nm_constant fields exactly so flat surfaces have zero tangential gradient.
...
void renderPerspective(vec2 uv) {
    ...
    // FILTERING is injected as a nm_constant when the runtime compiles a variant.
    if (FILTERING == 0) {
