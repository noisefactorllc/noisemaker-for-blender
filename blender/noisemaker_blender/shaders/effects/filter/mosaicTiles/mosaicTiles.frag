 ...
 *                 tile shows a mildly center-flattened nm_sample of its own
 ...
 *                 maxOffset% of a tile width, nm_constant across the whole
 ...
 * fixed structural nm_constant rather than a per-mode headline control, but
...
void main() {
    ...
    vec4 srcHome = nmTex(inputTex, uv);
    ...
    if (mode == 0) {
        ...
        // Flattened nm_sample position: bias this pixel's (warped) position
        ...
        // nm_sample "cell-clamped" per the task spec: it can never cross
        ...
        // at the tileSize noise period, so reusing it as a nm_constant
        ...
        vec3 tileColor = nmTex(inputTex, sampleUV).rgb;
        ...
        vec3 tileColor = nmTex(inputTex, shiftedUV).rgb;
