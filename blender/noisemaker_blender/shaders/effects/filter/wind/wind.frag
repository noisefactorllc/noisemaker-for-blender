 ...
 * looking for the brightest nm_sample that exceeds this pixel's own
 * luminance by threshold/100. That nm_sample is painted back onto the
 ...
 * bands, so adjacent bands nm_sample from different points along the
 ...
 *
 * Sampling offsets are plain nm_kernel-style pixel offsets off
...
void main() {
    ...
    // L/2 so adjacent bands nm_sample different parts of the upwind
    // scanline.
    ...
    // uniform wash. +17.0 is an arbitrary decorrelation nm_constant, not a
    // uniform.
