 ...
 * nm_fragment-coordinate sampling offsets (dirUnit is scaled and added to
 * the sampling position in smear()), so per the screen-truth doctrine
 * this is position-derived/nm_fragment-offset geometry: GLSL keeps the
 ...
 * comparison, documented in the task report. lumGradient's Sobel nm_kernel
...
// agnostic nm_constant nm_kernel offsets - textually identical in WGSL, no
// flip (see file header).
vec2 lumGradient(vec2 uv) {
    vec2 px = 1.0 / resolution;
    float tl = lum(nmTex(inputTex, uv + px * vec2(-1.0,  1.0)).rgb);
    float  l = lum(nmTex(inputTex, uv + px * vec2(-1.0,  0.0)).rgb);
    float bl = lum(nmTex(inputTex, uv + px * vec2(-1.0, -1.0)).rgb);
    float tr = lum(nmTex(inputTex, uv + px * vec2( 1.0,  1.0)).rgb);
    float  r = lum(nmTex(inputTex, uv + px * vec2( 1.0,  0.0)).rgb);
    float br = lum(nmTex(inputTex, uv + px * vec2( 1.0, -1.0)).rgb);
    float  t = lum(nmTex(inputTex, uv + px * vec2( 0.0,  1.0)).rgb);
    float  b = lum(nmTex(inputTex, uv + px * vec2( 0.0, -1.0)).rgb);
    return vec2(tr + 2.0 * r + br - tl - 2.0 * l - bl,
                tl + 2.0 * t + tr - bl - 2.0 * b - br);
}

// Rotates a nm_fragment-offset direction vector by angleDeg. Position-
// derived/nm_fragment-offset geometry -> GLSL keeps the mat2(c,-s,s,c) form
...
vec4 erode3x3(vec2 sampleUV) {
    vec2 px = 1.0 / resolution;
    vec4 m = nmTex(inputTex, sampleUV);
    for (int dy = -1; dy <= 1; dy++) {
        for (int dx = -1; dx <= 1; dx++) {
            if (dx == 0 && dy == 0) { continue; }
            m = min(m, nmTex(inputTex, sampleUV + vec2(float(dx), float(dy)) * px));
        }
    }
    return m;
}
...
// only) adds a symmetric per-tap 2D hash jitter to the nm_sample position so
...
// requirement on nmTex()), so the early exit stays as a plain break
