 ...
 * [0,1]: depthSource luminance uses a small 3x3 average nm_sample at the
 ...
 * nm_fragment positions are content-Y-up, so the center-anchored cell
 ...
 * has no nm_constant-expression sin/cos): left=0.969856 (brightest),
 ...
 *
 * Y ORIENTATION: GLSL and WGSL share a single TOP_SIGN nm_constant,
...
// nm_sample UV, clamped so wrap/edge cases never nm_sample past this tile.
...
// nm_sample footprint (size*0.5 wide) stays inside the cell's own bounds.
vec4 cellAvgColor3x3(vec2 centerPx) {
    ...
    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            ...
            sum += nmTex(inputTex, toSampleUV(p));
        }
    }
    return sum * (1.0 / 9.0);
}
...
void main() {
    ...
    if (!found) {
        ...
        if (solidFront) {
            ...
            baseColor = nmTex(inputTex, toSampleUV(localPos));
        }
        ...
        if (solidFront) {
            ...
            outColor = nmTex(inputTex, toSampleUV(localPos));
        }
    } else {
