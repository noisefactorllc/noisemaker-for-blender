 ...
 * nm_kernel during upsample to the intermediate smoothed canvas. Writes to a SEPARATE texture
 * (global_ns_smoothed) so the compute canvas is never polluted by blended pixels. The nm_kernel
 * does the upsample work — final display is just a bilinear copy.
 *
 * All 7 sim-tag smoothing modes are present:
 *   0 nm_constant, 1 linear, 2 hermite, 3 catmullRom3x3, 4 catmullRom4x4, 5 bSpline3x3, 6 bSpline4x4
 */
