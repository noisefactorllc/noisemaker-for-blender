...
*Incrementally synced 2026-09-24 to reference `4891b995642e` (`13fa8b540025..4891b995642e`) — ported upstream structured call-form and expectation diagnostics (`P007` invalid call form, `P001` remaining expectation diagnostics) into `compiler/parser.py` and `compiler/lang_data.py`. Added unit test suite in `parity/test_compiler.py`. Verified: `parity/compiler/check_{lex,parse,compile,expanded,graph}.py` all pass (20/20 lex/parse/compile, 19/19 expand/graph with `B5oBsA` compile-error exclusion), and 133/133 parity unit tests pass.*
...
*Incrementally synced 2026-09-25 to reference `2f47612c2904` (`9d3474dfdc6c..2f47612c2904`; the job's declared `fca611fd8f91..2f47612c2904` range was audited — its `fca611fd..9d3474df` shader delta is covered by the previous two entries, so the new delta is exactly the three commits `a021a283`/`62eb56fa`/`2f47612c` plus docs-only `13a8a049` (LEDGER.md/llms-full.txt, no shaders); range provenance without ancestry assumption — both `fca611fd8f91` and `9d3474dfdc6c` verified as ancestors of `2f47612c2904` via `git merge-base --is-ancestor` against a fresh upstream `main` clone, so the force-push-flagged declared range is contiguous and its `fca611fd..9d3474df` shader delta is covered by the previous two entries; `git diff --stat 9d3474df 2f47612c -- shaders/effects shaders/glsl shaders/src/lang` is empty (compare: https://github.com/noisefactorllc/noisemaker/compare/9d3474dfdc6c...2f47612c2904); the code commits touch only `shaders/src/runtime/{pipeline.js,compiler.js,effect-validator.js,backends/webgl2.js,backends/webgpu.js}` and `shaders/tests/test_mip_controls.js`) — audited upstream GAP-004 authorable `mipmaps`/`persistent` (2D) and `filter` (3D) texture policies plus its two follow-up fixes (per-level WebGL2 mip-chain allocation with cached WebGPU mip bind groups; global-surface double-create fix in `createSurfaces()`). The new texture-spec fields are opt-in and set by no effect definition (grep over all 210 upstream `definition.js` files finds no `mipmaps:`/`persistent:`/`filter:` texture-spec key). Confirmed inapplicable to the Blender runtime: the port renders single-mip-level targets (`nmTexLod` ignores `lod` by design, so per-frame `generateMipmaps` has no analogue or consumer), no graph spec carries `persistent`, and the Python `GpuBackend.setup()` has no double-create path — a changed global-surface allocation frees both halves and then recreates each exactly once (unchanged allocations `continue`), so upstream's orphaned-texture bug cannot occur here. Extraction-time definition validation (loaded live from the reference by `tools/convert-defs-blender.mjs`) now enforces the new texture-spec field rules; re-ran extraction at `2f47612c2904` — all 210 definitions validate and the emitted effect JSONs are byte-identical (no diff). Verified: compiler goldens regenerated unchanged (`parity/compiler/check_{lex,parse,compile,expanded,graph}.py` all pass, 20/20 lex/parse/compile, 19/19 expand/graph with `B5oBsA` compile-error exclusion), and 140/140 parity unit tests pass.*

*Incrementally synced 2026-09-25 to reference `8eeb7b5ac14e` (`2f47612c2904..8eeb7b5ac14e`, v1.0.183) — ported upstream GAP-005 pass field propagation (`fa83eeab`: pass labels and execution controls `name`, `type`, `clear`, `viewport`, `samplerTypes` copied verbatim from `passDef` onto expanded pass in `compiler/expander.py`). Updated `tools/convert-defs-blender.mjs` to preserve `viewport` and `samplerTypes` during effect extraction, updating 10 3D effect definitions (`synth3d` and `filter3d`) with authored viewport specifications. Updated `compiler/compiler.py` and `tools/export-graph.mjs` to preserve `viewport` and `samplerTypes` in normalized graph passes. Added `resolve_dimension` and `resolve_pass_viewport` in `runtime/pipeline.py` resolving pass viewports into `{ x, y, w, h }` integer boxes on `viewportResolved`, mirroring reference `Pipeline.resolvePassViewport`. Updated `backend/gpu_backend.py` to honor `viewportResolved`/`viewport` and `clear` during pass execution. Added test coverage in `parity/test_compiler.py`. Compiler goldens regenerated: all 5 compiler parity gates pass (20/20 lex/parse/compile, 19/19 expand/graph with `B5oBsA` compile-error exclusion); 141/141 parity unit tests pass.*
