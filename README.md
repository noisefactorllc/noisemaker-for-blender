...
| P1 shader transpiler | ✅ 249/249 transpiled; **255/257 programs compile on Metal — ALL but `scope`/`spectrum` (audio input, out of scope)**. `compile_check.py` now covers every path (fragment, **vertex+fragment**, **MRT**, **define-gated** via the effect's default defines; `skip=0`). Load-time MSL fixups: builtin-shadow rename, C++ alt-token rename, type-aware vecN==, mat2 vec-ctor, struct constructors, const-array params, prototype strip, scalar reflect/refract, v_texCoord→gl_FragCoord, multi-uniform-per-line split, std140 UBO + explicit named-block |
...
| **classicNoisedeck namespace** | ✅ **20/20 compile** (colorLab/bitEffects/fractal + `shapeMixer` fixed; `cnd_noise/shapes/fractal/bitEffects/caustic/moodscape/noise3d` **byte-identical**) |
| **deep-tail compile fixes** (dither, shapeMixer, mashup, remap) | ✅ **all 4 byte-exact vs reference golden** (max-diff 0.000): dither (const-array params + drop redundant prototype), shapeMixer (scalar reflect/refract → injected scope-aware nm_reflect/nm_refract), mashup (transpiler splits multi-uniform-per-line decls), remap (explicit `layout(std140) uniform { vec4 data[267]; }` zone-config UBO bound alongside push constants, packed via ported `packUniformsWithLayout`; default + zone-triangle both byte-exact). De-risked by `spike_dither/reflect/remap_ubo.py` |
| P6 attractor/lenia/life chaotic-precision | staged |
