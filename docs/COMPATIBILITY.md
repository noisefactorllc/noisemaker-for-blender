...
Scope: GAP-002's landscape filtering choices. For each of default, voxel, and isosurface, `parity/programs/heightmap3d_landscape.dsl` (with an explicit `filtering:` argument added for the two named choices; the base program is unchanged) was compiled in-Blender and rendered through `blender/harness/render_all.py` (256x256, NM_TIME 0.25, NM_FRAMES 8) and compared against the unchanged reference engine's own GPU renders (WebGL2/ANGLE SwiftShader via the same determinism protocol as `parity/batch-golden.mjs`, provenance resolved to the port's declared authority `8eeb7b5ac14e`, v1.0.183) at the existing tolerances (tol=2.0, ssim_min=0.98):

| Case | vs reference-engine render (WebGL2/SwiftShader) |
|---|---|
| `renderLandscape3d` default | measured: max-abs-diff=242.000 mean-abs-diff=0.1402 ssim=0.99756 — max-abs gate FAIL (175/65536 pixels over) |
| `renderLandscape3d` voxel | measured: identical to default on both engines (reference goldens byte-identical; candidate renders byte-identical) |
| `renderLandscape3d` isosurface | measured: max-abs-diff=244.000 mean-abs-diff=0.2583 ssim=0.99407 — max-abs gate FAIL (343/65536 pixels over) |

Divergence classification (measured, not asserted): a raw float32 readback of the shared `node_4_volumeCache` atlas on both engines (identical byte-equal-up-to-orientation `o1` height input, identical graph, both engines individually deterministic across repeated runs) shows 33110/262144 atlas cells differing by sub-1/255 FP noise and exactly 663 cells with a full voxel presence/color flip (alpha 0 vs 1; `analysis.json: volumeCache_raw_float_compare`), all other differences <= 2/255. The final-image over-gate pixels are single-pixel flips inside the voxel-grid terrain shading caused by those 663 flipped voxels (their mean local gradient is 32.6 vs an 8.1 image average), with overall mean-abs-diff <= 0.26 and ssim >= 0.994. The flips arise inside the landscape program's ray/voxel stepping from cross-engine FP divergence between llvmpipe GLSL and ANGLE/SwiftShader; the port's shaders are shared with the reference engine, so the divergence is not correctable in the port without changing the shared shader semantics. The strict max-abs gate remains failed at unchanged tolerances; no tolerance or golden changed.

Archive render (kit `0.1.18`): the packed engine (`engine/noisemaker_blender.zip`, SHA-256 verified against its immutable `kit.json`) was extracted into an isolated tree with the render harness and executed the same way: its own packed compiler compiles all three choices (FILTERING 1/1/0, `packed-kit-0.1.18-landscape-graphs.json`), but its renders are all-black for every mode (max-abs-diff 254/254/255, mean-abs ~21.5 vs the reference renders) because the distributed engine still contains the vec3-uniform defect fixed in this commit — the same all-black gradient probe reproduces on the archive. The published `0.1.22` kit shares that engine lineage, so no distributed archive renders these modes against authority today; a kit built from this commit's source is required for an archive-side pass.

The runs exposed a real port defect, fixed in the same commit as this record: `GpuBackend._set_uniform` passed a 4-component color value to Blender's `uniform_float` for a declared `vec3` uniform; Blender raises `ValueError`, which the handler swallowed, so every vec3 color uniform silently stayed zero and `synth/gradient` (and any landscape mode consuming it) rendered all-black. Pre-fix gradient probe: all-black (max-abs-diff 255 vs the reference render); post-fix: PASS at tol=2.0 (max-abs-diff=1.000, ssim=0.99999). The setter now normalizes any sequence to the declared vecN width, and a swallowed assignment prints a one-time `NMR WARN` line (benign for uniforms the shader compiler optimized out) instead of staying fully silent; under-length values are skipped with that warning rather than passed. The regression coverage in `blender/harness/test_backend_contract.py` was extended accordingly (over-length list/tuple/numpy sequences, under-length skip-with-warning, optimized-out uniform skip-with-once-warning); the committed log ends with a clean exit 0. Raw evidence for this run is committed under `parity/evidence-2026-09-26/` (candidate and reference-engine PNGs for all three modes, graph JSONs, compare reports, `analysis.json` including the raw volumeCache atlas comparison, compiler-gate and unit logs, the packed kit compiler probe, the kit archive render candidates and reports, and the Blender logs).
