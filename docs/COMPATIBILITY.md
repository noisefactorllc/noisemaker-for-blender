...
Divergence classification: the over-gate pixels are isolated single-pixel flips inside the voxel-grid terrain shading (silhouette/quantization boundaries; their mean local gradient is 32.6 vs an 8.1 image average), with overall mean-abs-diff <= 0.26 and ssim >= 0.994. The engines' `o1` noise height textures are byte-equal up to the engines' opposite FBO read orientations (flipud max-abs-diff=1.0; unflipped 192), so the height input is not the source; the flips arise inside the landscape program's ray/voxel stepping, consistent with cross-engine FP divergence between llvmpipe GLSL and ANGLE/SwiftShader. The strict max-abs gate remains failed at unchanged tolerances; no tolerance or golden changed.

The runs exposed a real port defect, fixed in the same commit as this record: `GpuBackend._set_uniform` passed a 4-component color value to Blender's `uniform_float` for a declared `vec3` uniform; Blender raises `ValueError`, which the handler swallowed, so every vec3 color uniform silently stayed zero and `synth/gradient` (and any landscape mode consuming it) rendered all-black. Pre-fix gradient probe: all-black (max-abs-diff 255 vs the reference render); post-fix: PASS at tol=2.0 (max-abs-diff=1.000, ssim=0.99999). A vecN-slicing regression test was added to `blender/harness/test_backend_contract.py`. Raw evidence for this run is committed under `parity/evidence-2026-09-26/` (candidate and reference-engine PNGs for all three modes, graph JSONs, compare reports, `analysis.json`, compiler-gate and unit logs, the packed kit compiler probe, and the Blender logs).
