...
- Status update 2026-09-26: open — compile checks complete; renders measured at unchanged tolerances with a residual edge-pixel divergence, so the render half of the acceptance is not met. Executed on the same unqualified Linux llvmpipe host class as the 2026-09-25 GAP-001 run (explicit limit; not GAP-003-qualified). Compiler tests: `parity/compiler/check_{lex,parse,compile,expanded,graph}.py` all pass (20/20 lex/parse/compile, 19/19 expand/graph with the `B5oBsA` exclusion) against the reference engine at the port's declared authority `8eeb7b5ac14e` (v1.0.183), and 143/143 parity unit tests pass. Packed compiler graphs: kit `0.1.18` (source `e9299fd8af7d546d27491959677211e8e89b1dc1`) `engine/noisemaker_blender.zip` verified against its immutable `kit.json` SHA-256, extracted in isolation, and its own packed compiler compiles all three choices with `FILTERING` 1/1/0 (default/voxel/isosurface), matching the checkout compiler (`parity/evidence-2026-09-26/packed-kit-0.1.18-landscape-graphs.json`). The earlier rejection evidence is preserved unchanged: the audited source `0efbdc47` rejected both named choices before the `44bc4ed4ac72` sync added them, and nothing here re-labels that record. Source-bound renders (details and full measurement table in [COMPATIBILITY.md](COMPATIBILITY.md#native-observations-2026-09-26)): each choice's DSL compiled in-Blender and rendered via `blender/harness/render_all.py` (256x256, time 0.25, 8 frames), graded against the unchanged reference engine's own GPU renders (WebGL2/SwiftShader, provenance `8eeb7b5ac14e`) at the unchanged tolerances (tol=2.0, ssim_min=0.98): default and voxel are byte-identical to each other on both engines and measure mean-abs-diff 0.1402 / ssim 0.99756 with 175/65536 pixels over the max-abs gate (max 242); isosurface measures mean-abs-diff 0.2583 / ssim 0.99407 with 343/65536 pixels over the gate (max 244). Both source revisions are recorded above (audit source `0efbdc47`, kit source `e9299fd8`, current authority `8eeb7b5ac14e`); the strict max-abs gate remains failed at unchanged tolerances, no tolerance or golden changed. The runs exposed a real port defect, fixed in this commit: `GpuBackend._set_uniform` fed a 4-component color value to a declared `vec3` uniform, Blender raised `ValueError`, and the swallowed error left every vec3 color at zero — `synth/gradient` and all three landscape modes rendered all-black before the fix (probe: all-black pre-fix; PASS max-abs-diff=1.0 post-fix). vecN-slicing regression test added in `blender/harness/test_backend_contract.py`. Raw outputs committed under `parity/evidence-2026-09-26/`. Remaining limits: unqualified host; full rendered parity unverified; GAP-002 stays open pending a GAP-003-qualified host or an accepted classification of the cross-engine edge-pixel divergence.
