...
- Status update 2026-09-25: not closed — measured evidence recorded; required checks unmet, so the conflicting-statement contradiction is resolved in the user-facing text but the gap stays open. Executed on a Linux Blender 5.1.2 host (OpenGL via Mesa llvmpipe under a virtual display). Public bake path: `test_integration.py` returned INTEGRATION PASS but PARTIAL — INVARIANT A (bake == direct pipeline) max-abs-diff=0; INVARIANT B (baked PNG vs the derived adjust golden) was skipped because that golden is absent on this host. Individual `bloom`, individual `lens` (three-line program quoted in the compatibility record), and the `north_star` chain (subchain `dpxp` with `bloom(taps: 15)` and `lens(displacement: -0.28)`) ran through `blender/harness/render_all.py` as golden (graph exported by the unchanged reference engine at the synced revision `2f47612c2904`) versus candidate (same DSL compiled in-Blender), graded by `parity/compare.py` at the existing tolerances (tol=2.0, ssim_min=0.98) and at exact zero tolerance: all three byte-identical (max-abs-diff=0.000, ssim=1.00000). Required-check status: public bake path partially met (INVARIANT A only); unchanged authority images NOT met (no retained authority golden reachable from this host — none was used or altered); existing tolerances met; individual effects and a chain met. The host is not GAP-003-qualified (GAP-003 stays open). The 2026-09-24 retained-golden `bloom` exact-comparison difference (max-abs-diff=1.000, tol=0.0) remains open, unexplained and unreproduced. Descriptions reconciled without claiming verification: the export README template no longer describes bloom and lens as "known broken"; it now states they are in the supported set, were measured against reference-derived graphs on 2026-09-25, and that an unresolved exact-comparison difference against the retained authority golden stays recorded as an open defect in the compatibility record. Remaining next action: repeat the same fixture set on a GAP-003-qualified host with the retained authority goldens (or resolve their provenance) before closing.
