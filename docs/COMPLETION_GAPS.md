...
- Required checks: install/enable, quick start, material/compositor use, save/reopen, error recovery, cleanup, disable/remove — all executed on the recorded host. Keyboard interaction is not met by this run: only the no-custom-keymap half could be verified from a script (the addon registers no custom keymaps); interactive typing was not executed and remains an open item under Still open.
